...
# Only TCP web ports are blocked; blocking all ports would affect SSH and
# other management services unnecessarily.
BLOCK_TCP_PORTS = (80, 443)

# Paths to UFW persistent rules files (IPv4 and IPv6).
UFW_RULES_FILES = ('/etc/ufw/user.rules', '/etc/ufw/user6.rules')

# iptables-restore command prefixes used inside UFW rules files.
_UFW_IPTABLES_PREFIXES = ('-A ', '-I ')
